"""Load a PyTorch checkpoint and evaluate the model on N puzzles."""
...
from common import encode_clues, evaluate_puzzle
...
def load_checkpoint(ckpt_dir: str, model_cfg: TransformerConfig, device: torch.device):
    ...
    model.eval()
    print(f"Loaded checkpoint at step {ckpt['step']}", flush=True)
    return model


def generate_trace(
    model, device: torch.device, puzzle: str, temperature: float = 0.0,
    ...
    with torch.no_grad():
        for _ in range(n_empties):
            ...
            input_arr = torch.tensor([padded], dtype=torch.long, device=device)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate PyTorch model on Sudoku puzzles")
    ...
    model = load_checkpoint(args.ckpt_dir, model_cfg, device)
    ...
    for idx, (puzzle, solution) in enumerate(puzzles):
        ...
        trace = generate_trace(model, device, puzzle, args.temperature)
